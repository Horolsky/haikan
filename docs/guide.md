...
Haikan is an embeddable expression language for declarative, inspectable data transformation,
originally created for [Zenseact Model-Based Testing Framework](https://zenseact.github.io/zmbt-framework).

The language consists of keywords that can be parametrized and combined into a single expression, resulting in a pure `JSON -> JSON` function, which is evaluated by test model runners. The language belongs to a family of [tacit programming languages](https://en.wikipedia.org/wiki/Tacit_programming).
As it operates on JSON, certain elements may resemble the [jq language](https://jqlang.org/), however, *Haikan* focus more on a simpler syntax,
...
General expression syntax is `literal | keyword[(expression...)]`, which expands to following options:
 ...
 - `keyword`: builtin expression keyword from `haikan` package.
...
As a deviation from the pure functional paradigm, Haikan provide access to
...
```
2025-07-20T14:04:14.485827201Z DEBUG haikan::Dbg
           ┌── Add $ [1,2] = 3
           ├── Add $ [3,3] = 6
           ├── Add $ [6,4.25E1] = 4.85E1
...
It can be combined with `HAIKAN_CUR_LOC` macro to trace mock invocations:

```c++
auto const f = Trace(HAIKAN_CUR_LOC) | Reduce(Add) & Size | Div;
auto const x = L{1,2,3,42.5};
```

outputs `<source-file>#<line-no> [1,2,3,4.25E1]` to the log.
...
```
2025-07-21T20:55:32.037990154Z DEBUG haikan::Dbg
     ┌── Trace("bar") $ 42 = 42
     ├── Sub(2) $ 42 = 40
  □  Trace("bar") | Sub(2) $ 42 = 40

2025-07-21T20:55:32.038143599Z DEBUG haikan::Dbg
     ┌── Trace("foo") $ 40 = 40
     ├── Add(2) $ 40 = 42
     │      ┌── Trace("bar") $ 42 = 42
     │      ├── Sub(2) $ 42 = 40
...
This option can be disabled with `log_notrim` flag.
...
!!! warning
    ...
    The builtin keywords from `haikan` namespace are callable objects with static storage,
