  ...
  private:
    ...
    static decorated_type validate(T init_value)
    {
        ...
        if (std::isnormal(init_value) and (T {value} !=  init_value)) {
            boost::throw_exception(std::runtime_error("precision loss when creating precise<T>"));
        }
        return value;
    }
    ...
    static decorated_type validate_str_as(boost::json::string_view str)
    {
        ...
        if (end == str.cbegin() ) {
            boost::throw_exception(std::invalid_argument("precise<T> string parsing error"));
        }
...
/// @tparam T
template <class T>
constexpr ::haikan::impl::type_tag<precise<T>> Precise;


// Boost JSON conversion from precise<T>
template <class T>
void tag_invoke(boost::json::value_from_tag const&, boost::json::value& v, precise<T> const& t)
{
    // v = t.stringify();
    v = t.value();
}

// Boost JSON conversion to precise<T>
template <class T>
precise<T> tag_invoke(boost::json::value_to_tag<precise<T>> const&, boost::json::value const& v)
{
    precise<T> result;
    switch (v.kind()) {
        ...
        case boost::json::kind::uint64:
            ...
            break;
        default:
            boost::throw_exception(std::invalid_argument("precise<T> conversion failure"));
            break;
    }
    return result;
}
