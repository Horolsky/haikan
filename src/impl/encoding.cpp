...
EncodingView::ExprRow EncodingView::at(std::size_t i) const
{
    if (i >= size_)
    {
        boost::throw_exception(std::range_error("Out of bounds EncodingView::at"));
    }
    ...
    {
        {
            ...
            {
                if (keyword_attributes(kwrds[i]) & attr::is_sideeffect)
                {
                    return false;
                }
            }
...
// Boost JSON conversion from Encoding
void tag_invoke(boost::json::value_from_tag const&, boost::json::value& v, Encoding const& enc)
{
    v = EncodingView(enc).to_json();
}

// Boost JSON conversion to Encoding
Encoding tag_invoke(boost::json::value_to_tag<Encoding> const&, boost::json::value const& v)
{
    return Encoding(v);
}
