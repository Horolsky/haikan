    ...
    if (auto const delete_range = at.if_array())
    {
        ...
        else
        {
            ...
            for (auto const& el: *delete_range)
            {
                ...
                else if (el.is_number()) sorted_delete_range.push_back((boost::format("/%d") % el).str().c_str());
            }
        }
        ...
        std::stable_sort(
            ...
            [](auto const& a, auto const& b){
                return haikan::impl::Operator::generic_less(a, b).as_bool();
            }
        );
...
HAIKAN_DEFINE_EVALUATE_IMPL(Del)
{
