...
boost::json::value query_at_impl(boost::json::value const& value, boost::json::value const& at)
{
        ...
        for (auto const& kv: *at_as_object)
        {
            if (kv.key().starts_with("$")) // dynamic key as ptr
            {
                try
                {
                    ...
                    {
                        dynamic_key = (boost::format("%s") % dynamic_key).str();
                    }
    ...
    else if (at_as_string)
    {
        ...
        else if (value_as_array)
        {
            ...
            result = haikan::impl::slice(*value_as_array, slice_idx.at(0),slice_idx.at(1),slice_idx.at(2));
        }
        else if (value_as_string)
        {
            auto const slice_idx = haikan::impl::str_to_slice_idx(token);
            result = haikan::impl::slice(*value_as_string, slice_idx.at(0),slice_idx.at(1),slice_idx.at(2));
        }
        else if (value_as_object)
        {
...
boost::json::value query_at(ExpressionView const& value, ExpressionView const& query, haikan::impl::EvalContext ctx)
{
...
HAIKAN_DEFINE_EVALUATE_IMPL(At)
{
