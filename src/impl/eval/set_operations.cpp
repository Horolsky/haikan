...
HAIKAN_DEFINE_EVALUATE_IMPL(Cartesian)
{
...
HAIKAN_DEFINE_EVALUATE_IMPL(Uniques)
{
...
HAIKAN_DEFINE_EVALUATE_IMPL(Intersect)
{
    ASSERT(lhs().if_array(), "invalid argument");
...
HAIKAN_DEFINE_EVALUATE_IMPL(Diff)
{
    ASSERT(lhs().if_array(), "invalid argument");
    ASSERT(rhs().if_array(), "invalid parameter");
    auto const lhs_set = boost::json::value_to<std::unordered_set<V>>(lhs().data());
...
HAIKAN_DEFINE_EVALUATE_IMPL(Union)
{
    ASSERT(lhs().if_array(), "invalid argument");
    ASSERT(rhs().if_array(), "invalid parameter");
    auto const lhs_set = boost::json::value_to<std::unordered_set<V>>(lhs().data());
