...
HAIKAN_DEFINE_EVALUATE_IMPL(Tuple)
{
    return Expression(self().subexpressions_list());
}

HAIKAN_DEFINE_EVALUATE_IMPL(Pipe)
{
...
HAIKAN_DEFINE_EVALUATE_IMPL(Fork)
{
...
HAIKAN_DEFINE_EVALUATE_IMPL(Fmt)
{
    auto const subexpressions = self().subexpressions_list();
    ...
    else if (if_arr && subexpressions.empty())
    {
        ...
        {
            args.push_back(Expression(arg).eval({}, curr_ctx()));
        }
    }
    else
    {
...
HAIKAN_DEFINE_EVALUATE_IMPL(All)
{
    auto const subexpressions = self().subexpressions_list();
    E err_sts(nullptr);
...
HAIKAN_DEFINE_EVALUATE_IMPL(Any)
{
    auto const subexpressions = self().subexpressions_list();
    E err_sts(nullptr);
...
HAIKAN_DEFINE_EVALUATE_IMPL(Saturate)
{
    auto const subexpressions = self().subexpressions_list();
    auto const& x = lhs().data();
