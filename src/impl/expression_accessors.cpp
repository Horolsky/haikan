    ...
    for(auto const item: encoding_view())
    {
        ...
        else if((Keyword::_Literal == item.keyword) && item.data)
        {
            JsonTraverse([&](boost::json::value const& node, std::string const jp) -> bool {
                ...
                {
                    pp.emplace_back(node.as_string().c_str(), (boost::format("/data/%d%s") % item.index % jp).str());
                }
                return false;
            })(*item.data);
        }
...
boost::json::value const& ExpressionView::data() const
{
    ...
    return encoding_view().empty() ? Noop.data() : *encoding_view().front().data;
}
...
std::string ExpressionView::error_id() const
{
    ...
    return (boost::format("$(%s-%s-%s)") % type_name % hash(msg) % hash(ctx)).str();
}
