...
try
{
    ...
    default:
        return make_error_expr("unsupported operator", BOOST_CURRENT_FUNCTION);
    }
}
catch(const std::exception& e)
{
    return make_error_expr(e.what(), annotation());
}
...
boost::json::value Operator::is_subset(LV const& llhs, LV const& lrhs) const
{
    ...
    if (lhs.is_array() && rhs.is_array())
    {
        ...
        for (auto const& item_a: a)
        {
            ...
            {
                return make_error_expr("invalid operands", BOOST_CURRENT_FUNCTION);
            }
        }
        return true;
    }
