...
#define RETURN_ERROR(msg) return ::haikan::impl::make_error_expr(msg, BOOST_CURRENT_FUNCTION);
    ...
    bool is_less_as_array(boost::json::array const& lhs, boost::json::array const& rhs)
    {
        return std::lexicographical_compare(
            ...
            [](auto const& l, auto const& r) {
                return haikan::impl::Operator::generic_less(l, r).as_bool();
            }
        );
    }
...
V Operator::generic_pow(LV llhs, LV lrhs)
{
    ...
    {
        auto x = boost::json::value_to<double>(lhs);
        auto y = boost::json::value_to<double>(rhs);
        ret = boost::json::value_from(std::pow(x, y));
    }
    else
    {
        auto x = boost::json::value_to<std::complex<double>>(lhs);
        auto y = boost::json::value_to<std::complex<double>>(rhs);
        ret = boost::json::value_from(std::pow(x, y));
    }
...
V Operator::generic_log(LV llhs, LV lrhs)
{
    ...
    if (lhs.is_number() && rhs.is_number())
    {
        auto x = boost::json::value_to<double>(lhs);
        auto y = boost::json::value_to<double>(rhs);
        ret = boost::json::value_from(std::log(x) / std::log(y));
    }
    else
    {
        auto x = boost::json::value_to<std::complex<double>>(lhs);
