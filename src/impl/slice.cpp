...
safe_slice_t get_safe_slice(std::size_t const N, slice_t slice)
{
    ...
    if (step == 0)
    {
        boost::throw_exception(std::runtime_error("slice step cannot be zero"));
    }
...
boost::json::array slice(boost::json::array const& src, boost::json::string_view jp, std::int64_t const start, std::int64_t const stop, std::int64_t const step)
{
    ...
    while ((element = gen()) != src.cend())
    {
        ...
        if (!subelement)
        {
            boost::throw_exception(std::runtime_error((boost::format("slice error: %s not found at %s") % jp % *element).str()));
        }
...
std::array<std::int64_t, 3> str_to_slice_idx(boost::json::string_view slice_expr)
{
    ...
    {
        boost::throw_exception(std::runtime_error((boost::format("invalid slice: \"%s\"") % slice_expr).str().c_str()));
    }
    ...
    {
        boost::throw_exception(std::runtime_error((boost::format("invalid slice: \"%s\" , error: `%s`") % slice_expr % e.what()).str().c_str()));
    }
